...
def greedy2(filename):
    ...
    #print(activities)
    for i in range(len(activities)): # for each activity
        time_thing = activities[i][3]/(activities[i][1]) # calculates enjoyment per hr
        activities[i] = list(activities[i])
        activities[i].append(time_thing)
        budg_thing = activities[i][3]/(activities[i][2]) # calculates enjoyment per pound
        activities[i].append(budg_thing)
        activities[i] = tuple(activities[i])
       # for each activity adds its score for both onto end
    activities_chosen = []
    act_copy = activities.copy()
    sum_t = 0
    sum_c = 0
    while True: # loops until list is empt
        allowed = True # default activity to be allowed to be added 
        best = [-1,-1] # null position to hold the best activity at each point
        if sum_t < (max_time*(3/4)): # checks if sum of current time is less than 3/4 time allowed
             # will add activity based on  enjoyment/time
            for i in range(len(act_copy)): # each activity in list
                #print(act_copy[i][4])
                if act_copy[i][4] > best[0]: # if new activity enj/time is better than current best 
                    best = [act_copy[i][4],i] # set it as new best
        else: # if in last 1/4
            for i in range(len(act_copy)):
                #print(act_copy[i][4])
                if act_copy[i][5] > best[0]: # finds highest enj/cost remaining
                    best = [act_copy[i][5],i]
        if (act_copy[best[1]][1] + sum_t) > max_time:
            allowed = False # checks if activity added is within time constraint
        # if (act_copy[best[1]][2] + sum_c) > max_budget:
        #     allowed = False
        if allowed == True: # if its allowed
            activities_chosen.append(act_copy[best[1]]) # add it to activities list
        copy = []
        #print(activities_chosen)
        #print("Im removing activity ,",act_copy[best[1]],"it was used:",allowed)
        for i in range(len(act_copy)): # recreat act_copy removing the activity wether it wa allowed or not
            if i != best[1]:
                copy.append(act_copy[i])
        act_copy = copy
        #print(act_copy)
        # recaulculate current total time and cost 
        sum_t = 0
        sum_c = 0
        for i in activities_chosen:
            sum_t += i[1]
            sum_c += i[2]
        if sum_t >= max_time: # 
            break
        if len(act_copy) == 0: # when list is empty break
            break
    
    time_used = 0 # used to see how much time was usd in chosen path
    cost = 0 # used to calculate the total cost
    enjoyment = 0 # used to calculate totoal happiness
    for i in range(len(activities_chosen)): # loop through each activity used
        time_used += int(activities_chosen[i][1]) # add time on to total
        cost += int(activities_chosen[i][2]) # add cost on to total
        enjoyment += int(activities_chosen[i][3]) # add happiness on to total
    return activities_chosen, enjoyment, cost, time_used, max_budget,max_time
...
def final_output(file): # this is the function that creates the final output of the functions. 
    ...
    print("--- GREED ---")
    print("Selected Activities:")
    for i in range(len(greed[0])):
        print(f" - {greed[0][i][0]} ({greed[0][i][1]} hours, £{greed[0][i][2]}, enjoyment {greed[0][i][3]})")
    print('')
    print(f"Total Enjoyment: {greed[1]}")
    print(f"Total Time Used: {greed[3]} hours")
    print(f"Total Cost: £{greed[2]}\n")
    print(f"Execution time: {speed3:.4f} seconds")
